...
def calc_accuracy(data: pd.DataFrame, observed_col: str, simulated_col: str, threshold_col: str):
    # Test the input. Make sure that the DataFrame contains the required columns
    if not all(column in data.columns for column in [observed_col, simulated_col]):
        raise ValueError(f'DataFrame is missing one or more required columns: {observed_col, simulated_col}')
...
def calculate_RMSE(observed: np.ndarray, 
                  simulated: np.ndarray, 
                  normalize: bool = True) -> Union[float, Tuple[float, int]]:
    """
...
def calculate_MAE(observed: np.ndarray,
                  simulated: np.ndarray, 
                  normalize: bool = True) -> Union[float, Tuple[float, int]]:
    """
...
def calculate_QuantileLoss(observed: np.ndarray, 
    ...
    for i, q in enumerate(quantile_levels):
        pinball_loss[i] = np.mean(np.maximum(q * (valid_observed - valid_simulated[:, i]),
...
def calculate_crps_from_quantiles(quantile_levels, quantile_forecasts, observation):
    ...
    min_x = min(quantile_forecasts[0], observation) - 0.1 * (quantile_forecasts[-1] - quantile_forecasts[0])
    max_x = max(quantile_forecasts[-1], observation) + 0.1 * (quantile_forecasts[-1] - quantile_forecasts[0])
...
def calculate_metrics_pentad(df, forecast_col, observed_col, delta_col,):

    df = df.copy()
    metrics = pd.DataFrame()
    for pentad in df.pentad_in_year.unique():
        df_pentad = df[df['pentad_in_year'] == pentad].copy()
        accuracy = forecast_accuracy_hydromet(df_pentad, observed_col, forecast_col, delta_col).loc['accuracy']
        efficiency = sdivsigma_nse(df_pentad, observed_col, forecast_col).loc['sdivsigma']
        nse = sdivsigma_nse(df_pentad, observed_col, forecast_col).loc['nse']

        new_row = {'pentad_in_year': pentad, 'accuracy': accuracy, 'efficiency': efficiency, 'nse': nse}
...
def calculate_metrics_decad(df, forecast_col, observed_col, delta_col,):

    df = df.copy()
    metrics = pd.DataFrame()
    for decad in df.decad_in_year.unique():
        df_decad = df[df['decad_in_year'] == decad].copy()
        accuracy = forecast_accuracy_hydromet(df_decad, observed_col, forecast_col, delta_col).loc['accuracy']
        efficiency = sdivsigma_nse(df_decad, observed_col, forecast_col).loc['sdivsigma']
        nse = sdivsigma_nse(df_decad, observed_col, forecast_col).loc['nse']

        new_row = {'decad_in_year': decad, 'accuracy': accuracy, 'efficiency': efficiency, 'nse': nse}
