...
MODEL_FAMILIES = {
    ...
    'SnowMapper_Based': ['CondenseLR', 'LR_Q_SWE', 'LR_Q_SWE_T', 'LR_Q_T_P_SWE', 
                        'LR_Q_dSWEdt_T_P', 'LongTermLR', 'ShortTermLR', 'ShortTerm_Features']
}

# Configuration
EVALUATION_DAY_OF_MONTH = 'end'  # 'end' for last day of month, or integer for specific day
PREDICTION_FILENAME = 'predictions.csv'

def scan_prediction_files(results_dir: str = "../monthly_forecasting_results") -> Dict[str, List[str]]:
    """
    ...
    logger.info(f"Discovered {total_models} models across {len(discovered_models)} families")
...
def _standardize_prediction_columns(df: pd.DataFrame, model_name: str) -> pd.DataFrame:
    ...
    if 'Q_pred' in df.columns:
        ...
        if q_cols:
            ...
            logger.warning(f"Using {prediction_col} as prediction column for {model_name}")
...
def _filter_evaluation_dates(df: pd.DataFrame, 
                           evaluation_day: Union[str, int] = EVALUATION_DAY_OF_MONTH) -> pd.DataFrame:
    """
...
def load_predictions(model_paths: Dict[str, List[str]], 
                    evaluation_day: Union[str, int] = EVALUATION_DAY_OF_MONTH,
                    common_codes_only: bool = True) -> Dict[str, pd.DataFrame]:
    """
...
def get_model_family_mapping(loaded_predictions: Dict[str, pd.DataFrame]) -> Dict[str, List[str]]:
    """
...
def validate_prediction_data(loaded_predictions: Dict[str, pd.DataFrame]) -> Dict[str, Dict]:
    """
    ...
    for model_id, df in loaded_predictions.items():
        ...
        if results['missing_observations'] > 0:
            results['warnings'].append(f"{results['missing_observations']} missing observations")
        
        if results['missing_predictions'] > 0:
            results['warnings'].append(f"{results['missing_predictions']} missing predictions")
...
def load_all_predictions(results_dir: str = "../monthly_forecasting_results",
                        evaluation_day: Union[str, int] = EVALUATION_DAY_OF_MONTH,
                        common_codes_only: bool = True) -> Tuple[Dict[str, pd.DataFrame], Dict[str, Dict]]:
    """
    ...
    loaded_predictions = load_predictions(model_paths, evaluation_day, common_codes_only)
