...
def calculate_metrics(
    ...
    if len(obs_valid) < 2:
        logger.warning(
            f"Insufficient valid data for {model_id}: {len(obs_valid)} points"
        )
        return {
            "r2": np.nan,
            "rmse": np.nan,
...
def evaluate_per_code(
    df: pd.DataFrame, model_id: str = "unknown", min_samples: int = 5
) -> pd.DataFrame:
    """
    ...
    for code in df["code"].unique():
        ...
        if len(df_code) < min_samples:
            logger.warning(
                f"Insufficient data for code {code} in {model_id}: {len(df_code)} samples"
            )
            continue

        # Calculate metrics for this code
        metrics = calculate_metrics(
            df_code["Q_obs"], df_code["Q_pred"], f"{model_id}_code_{code}"
        )
...
def evaluate_per_month(
    df: pd.DataFrame, model_id: str = "unknown", min_samples: int = 3
) -> pd.DataFrame:
    """
    ...
    for month in sorted(df["month"].unique()):
        ...
        if len(df_month) < min_samples:
            logger.warning(
                f"Insufficient data for month {month} in {model_id}: {len(df_month)} samples"
            )
            continue

        # Calculate metrics for this month
        metrics = calculate_metrics(
            df_month["Q_obs"], df_month["Q_pred"], f"{model_id}_month_{month}"
        )
...
def evaluate_per_code_month(
    df: pd.DataFrame, model_id: str = "unknown", min_samples: int = 2
) -> pd.DataFrame:
    """
...
def evaluate_overall(
    df: pd.DataFrame, model_id: str = "unknown", min_samples: int = 10
) -> pd.DataFrame:
    """
    ...
    if len(df) < min_samples:
        logger.warning(
            f"Insufficient data for overall evaluation of {model_id}: {len(df)} samples"
        )
        return pd.DataFrame()
...
def evaluate_model_comprehensive(
    ...
    if include_code_month:
        ...
        code_month_results = evaluate_per_code_month(
            df, model_id, min_samples_code_month
        )
    ...
    if all_results:
        ...
        logger.info(
            f"Completed evaluation for {model_id}: {len(combined_results)} result rows"
        )
...
def evaluate_multiple_models(
    ...
    for model_id, df in loaded_predictions.items():
        ...
        model_name = (
            df["model_name"].iloc[0] if "model_name" in df.columns else model_id
        )
    ...
    if all_model_results:
        ...
        logger.info(
            f"Completed evaluation for {len(loaded_predictions)} models: {len(combined_results)} total result rows"
        )
        return combined_results
    else:
...
def calculate_model_rankings(
    ...
    return ranked_results[
        ["rank", "model_id", "family", "model_name", metric, "n_samples"]
    ].copy()
...
if __name__ == "__main__":
    ...
    # Evaluate all models
    results = evaluate_multiple_models(predictions, include_code_month=True)
