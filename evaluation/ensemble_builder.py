...
def create_simple_ensemble(prediction_data: List[pd.DataFrame],
    ...
    if weights is not None and len(weights) != len(prediction_data):
        logger.warning(f"Length mismatch: {len(weights)} weights for {len(prediction_data)} models")
        weights = None
...
def create_family_ensemble(loaded_predictions: Dict[str, pd.DataFrame],
                          family_name: str,
                          ensemble_method: str = 'mean',
                          weights: Optional[List[float]] = None) -> Optional[pd.DataFrame]:
    """
    ...
    logger.info(f"Creating ensemble for {family_name} with {len(family_predictions)} models: {family_model_ids}")
...
def create_global_ensemble(loaded_predictions: Dict[str, pd.DataFrame],
    ...
    logger.info(f"Creating global ensemble with {len(ensemble_data)} components: {source_models}")
...
def create_all_ensembles(loaded_predictions: Dict[str, pd.DataFrame],
                        ensemble_method: str = 'mean',
                        create_global: bool = True,
                        weights: Optional[Dict[str, List[float]]] = None) -> Tuple[Dict[str, pd.DataFrame], Dict[str, pd.DataFrame]]:
    """
...
def save_ensemble_predictions(family_ensembles: Dict[str, pd.DataFrame],
                             global_ensembles: Dict[str, pd.DataFrame],
                             output_dir: str = "../monthly_forecasting_results/evaluation") -> Dict[str, str]:
    """
...
def combine_predictions_and_ensembles(loaded_predictions: Dict[str, pd.DataFrame],
                                     family_ensembles: Dict[str, pd.DataFrame],
                                     global_ensembles: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    """
    ...
    logger.info(f"Combined {len(loaded_predictions)} individual models, "
                f"{len(family_ensembles)} family ensembles, "
                f"and {len(global_ensembles)} global ensembles")
    
    return combined_predictions

def create_ensemble_summary(family_ensembles: Dict[str, pd.DataFrame],
                           global_ensembles: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """
