...
try:
    ...
    from .ensemble_builder import (
        ...
        combine_predictions_and_ensembles,
    )
...
class EvaluationPipeline:
    ...
    def load_predictions(self) -> bool:
        ...
        try:
            ...
            total_warnings = sum(
                len(v.get("warnings", [])) for v in self.validation_results.values()
            )
            if total_warnings > 0:
    ...
    def _generate_metrics_summary(self) -> Dict:
        ...
        overall_results = self.evaluation_results[
            self.evaluation_results["level"] == "overall"
        ]
    ...
    def _generate_family_metrics(self) -> pd.DataFrame:
        ...
        family_stats = (
            overall_results.groupby("family")
            .agg(
                {
        ...
        self.metadata = {
            ...
            "validation_summary": {
                ...
                "models_with_warnings": sum(
                    1 for v in self.validation_results.values() if v.get("warnings")
                ),
            },
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monthly Discharge Forecasting Evaluation Pipeline"
    )
