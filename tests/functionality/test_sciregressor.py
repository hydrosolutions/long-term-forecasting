...
MODEL_CONFIG = {
    "xgb": {"n_estimators": 10, "max_depth": 2, "learning_rate": 0.1},
    "rf": {"n_estimators": 5, "max_depth": 2},
    "catboost": {"iterations": 20, "depth": 2, "learning_rate": 0.1},
}
    ...
    def generate_synthetic_timeseries_data(
        ...
        for code in basin_codes:
            for date in date_range:
                ...
                baseflow_seasonal = 20 * np.sin(
                    2 * np.pi * (day_of_year + 200) / 365.25
                )

                discharge = (
                    base_discharge + precip_effect + temp_effect + baseflow_seasonal
                )
            ...
            static_data.append(
                {
                    ...
                    "area_km2": 100
                    + np.random.uniform(50, 500),  # km² (for SciRegressor)
...
class SciRegressorTester:
    ...
    def setup_test_environment(self):
        ...
        self.data = (
            ComprehensiveTestDataGenerator.generate_comprehensive_timeseries_data()
        )
        self.static_data = (
            ComprehensiveTestDataGenerator.generate_comprehensive_static_data()
        )
    ...
    def test_feature_extraction(self):
        ...
        try:
            ...
            feature_cols = [
                ...
                if any(f in col for f in ["discharge", "P_", "T_"])
            ]
            ...
            with patch.object(SciRegressor, "__preprocess_data__") as mock_preprocess:
                with patch(
                    ...
                    with patch.object(SciRegressor, "load_model") as mock_load:
                        with patch.object(
                            SciRegressor, "predict_operational"
                        ) as mock_predict:
                                ...
                                mock_forecast_data.append(
                                    {
                                        ...
                                        "valid_to": "2025-08-03",
                                        "Q_TestSciRegressor": np.random.uniform(
                                            10, 100
                                        ),
                                    }
                                )
    ...
    def test_hyperparameter_tuning(self):
        ...
        try:
                ...
                with patch(
                    ...
                    with patch.object(
                        ...
                        mock_tune.return_value = (
                            True,
                            "Hyperparameters tuned successfully",
                        )
            ...
            assert isinstance(success, bool), (
                "Tuning should return boolean success status"
            )
...
class ComprehensiveSciRegressorTester:
    ...
    def _run_single_test(self, test_func, test_name: str, timeout: int = 120) -> bool:
        ...
        try:
            ...
            test_result = format_test_results(
                test_name, True, execution_time=execution_time
            )
            ...
            # Format and store results
            test_result = format_test_results(
                test_name, False, execution_time=execution_time
            )
            self.test_results.append(test_result)
    ...
    def test_specific_hyperparameter_tuning(
        self, model_type: str, preprocessing_method: str
    ) -> bool:
    ...
    def test_specific_calibration(
        ...
        try:
            ...
            with patch.object(model, "calibrate_model_and_hindcast") as mock_calibrate:
                ...
                mock_data[f"Q_{model.name}"] = mock_data["discharge"] * (
                    1 + np.random.normal(0, 0.1, len(mock_data))
                )
                mock_calibrate.return_value = mock_data.head(
                    100
    ...
    def test_specific_hindcast(
        ...
        try:
            ...
            with patch.object(model, "calibrate_model_and_hindcast") as mock_hindcast:
                ...
                mock_hindcast.return_value = mock_data.head(
                    100
                )  # Return subset for faster testing
    ...
    def test_specific_operational_prediction(
        self, model_type: str, preprocessing_method: str
    ) -> bool:
    ...
    def test_specific_complete_workflow(
        self, model_type: str, preprocessing_method: str
    ) -> bool:
                ...
                with patch.object(
                    ...
                    with patch.object(model, "predict_operational") as mock_predict:
                        with patch.object(model, "load_model") as mock_load:
                            ...
                            calibration_result = model.calibrate_model_and_hindcast()
                            assert len(calibration_result) > 0, (
                                "Calibration should return results"
                            )
    ...
    def test_specific_multi_model_ensemble(self, preprocessing_method: str) -> bool:
        ...
        try:
            ...
            for model_type in MODEL_TYPES:
                model, env = self._create_model_instance(
                    model_type, preprocessing_method
                )
            ...
            with patch.object(
                SciRegressor, "calibrate_model_and_hindcast"
            ) as mock_calibrate:
                ...
                for model_type in MODEL_TYPES:
                    mock_data[f"Q_{model_type}"] = mock_data["discharge"] * (
                        1 + np.random.normal(0, 0.1, len(mock_data))
                    )
    ...
    def run_comprehensive_tests(self) -> bool:
        ...
        component_tests = {
            "hyperparameter_tuning": (
                self.test_specific_hyperparameter_tuning,
                TEST_TIMEOUTS["hyperparameter_tuning"],
            ),
            "calibration": (
                self.test_specific_calibration,
                TEST_TIMEOUTS["calibration"],
            ),
            "hindcast": (self.test_specific_hindcast, TEST_TIMEOUTS["hindcast"]),
            "operational_prediction": (
                self.test_specific_operational_prediction,
                TEST_TIMEOUTS["operational_prediction"],
            ),
        }
        ...
        for model_type in MODEL_TYPES:
            for preprocessing_method in PREPROCESSING_METHODS:
                ...
                def test_wrapper():
                    return self.test_specific_complete_workflow(
                        model_type, preprocessing_method
                    )

                print(f"\n[{len(self.test_results) + 1}/{total_tests}] {test_name}...")
                self._run_single_test(
                    test_wrapper, test_name, TEST_TIMEOUTS["complete_workflow"]
                )
        ...
        for preprocessing_method in PREPROCESSING_METHODS:
            ...
            print(f"\n[{len(self.test_results) + 1}/{total_tests}] {test_name}...")
            self._run_single_test(
                test_wrapper, test_name, TEST_TIMEOUTS["multi_model_ensemble"]
            )
...
if PYTEST_AVAILABLE:
    ...
    def test_xgb_hyperparameter_tuning_global_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_hyperparameter_tuning(
            "xgb", "global_normalization"
        )
    ...
    def test_catboost_hyperparameter_tuning_global_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_hyperparameter_tuning(
            "catboost", "global_normalization"
        )

    def test_catboost_hyperparameter_tuning_per_basin_normalization(
        comprehensive_tester,
    ):
        """Test CatBoost hyperparameter tuning with per-basin normalization."""
        assert comprehensive_tester.test_specific_hyperparameter_tuning(
            "catboost", "per_basin_normalization"
        )

    def test_catboost_hyperparameter_tuning_long_term_mean_scaling(
        comprehensive_tester,
    ):
        """Test CatBoost hyperparameter tuning with long-term mean scaling."""
        assert comprehensive_tester.test_specific_hyperparameter_tuning(
            "catboost", "long_term_mean_scaling"
        )
    ...
    def test_xgb_calibration_global_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_calibration(
            "xgb", "global_normalization"
        )
    ...
    def test_xgb_hindcast_global_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_hindcast(
            "xgb", "global_normalization"
        )
    ...
    def test_xgb_operational_prediction_no_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_operational_prediction(
            "xgb", "no_normalization"
        )
    ...
    def test_catboost_operational_prediction_per_basin_normalization(
        comprehensive_tester,
    ):
    ...
    def test_catboost_operational_prediction_long_term_mean_scaling(
        comprehensive_tester,
    ):
    ...
    def test_xgb_complete_workflow_no_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_complete_workflow(
            "xgb", "no_normalization"
        )
    ...
    def test_multi_model_ensemble_no_normalization(comprehensive_tester):
        ...
        assert comprehensive_tester.test_specific_multi_model_ensemble(
            "no_normalization"
        )
