    ...
    def generate_synthetic_timeseries_data(
        ...
        for code in basin_codes:
            for date in date_range:
                ...
                discharge = base_discharge + precip_effect + temp_effect + np.random.normal(0, 5)
...
class LinearRegressionTester:
    ...
    def test_model_initialization(self):
        ...
        try:
            ...
            feature_cols = [col for col in model.data.columns if 'discharge' in col or 'P_' in col or 'T_' in col]
    ...
    def test_calibration_and_hindcast(self):
        ...
        try:
            ...
            logger.info(f"  Prediction range: {hindcast_df[pred_col].min():.2f} to {hindcast_df[pred_col].max():.2f}")
    ...
    def test_operational_forecast(self):
        ...
        try:
            ...
            # Verify results
            if len(forecast_df) > 0:  # Forecast might be empty if no data for current period
                # Check required columns
                ...
                logger.info(f"  Forecast range: {forecast_df[pred_col].min():.2f} to {forecast_df[pred_col].max():.2f}")
            else:
    ...
    def test_metrics_calculation(self, hindcast_df: pd.DataFrame):
        ...
        try:
            ...
            assert metrics_df['r2'].min() >= -10, "R2 values unreasonably low"  # Allow for some poor predictions
        ...
        try:
            ...
            if hasattr(model, 'fitted_models') and len(model.fitted_models) > 0:
                ...
                try:
                    ...
                    logger.warning(f"Model loading failed (may not be implemented): {e}")
            else:
...
def main():
    ...
    parser.add_argument('--log-file', type=str, default='test_linear_regression.log',
