...
class ComprehensiveTestDataGenerator:
    ...
    def generate_comprehensive_timeseries_data(
        ...
        for code in basin_codes:
            for date in date_range:
                ...
                T = 5 + TEST_DATA_PARAMS["seasonal_amplitude"] * np.sin(
                    2 * np.pi * day_of_year / 365.25
                )
                T += 0.02 * (year - 2000)  # Climate change trend
                T += TEST_DATA_PARAMS["elevation_effect"] * (
                    code - 16938
                )  # Elevation effect
                T += np.random.normal(
                    0, TEST_DATA_PARAMS["noise_factors"]["temperature"]
                )

                # Enhanced precipitation with seasonality, extremes, and basin effects
                P = max(
                    0,
                    TEST_DATA_PARAMS["precip_base"]
                ...
                basin_factor = (
                    1 + TEST_DATA_PARAMS["basin_scaling_factor"] * (code - 16938) / 4
                )
                P *= basin_factor
                P = max(
                    0,
                    P
                    + np.random.normal(
                        0, P * TEST_DATA_PARAMS["noise_factors"]["precipitation"]
                    ),
                )

                # Complex discharge model with multiple influences
                base_discharge = TEST_DATA_PARAMS["discharge_base"] + 30 * np.sin(
                ...
                discharge = max(
                    ...
                    discharge
                    + np.random.normal(
                        0, discharge * TEST_DATA_PARAMS["noise_factors"]["discharge"]
                    ),
                )
        ...
        logger.info(
            f"Generated comprehensive synthetic data: {len(df)} records for {len(basin_codes)} basins"
        )
    ...
    def generate_comprehensive_static_data(
        ...
        logger.info(
            f"Generated comprehensive static data for {len(basin_codes)} basins"
        )
        logger.info(f"Preprocessing method: {preprocessing_method}")
...
class ComprehensiveTestValidator:
    ...
    def validate_model_initialization(
        model, model_type: str, preprocessing_method: str
    ) -> bool:
        """
        ...
        try:
            ...
            assert hasattr(model, "static_data"), (
                "Model should have static_data attribute"
            )
            assert hasattr(model, "models"), "Model should have models attribute"
            assert hasattr(model, "fitted_models"), (
                "Model should have fitted_models attribute"
            )

            # Check model type
            assert model_type in model.models, (
                f"Model type {model_type} not in models list"
            )
            ...
            logger.info(
                f"Model initialization validation passed for {model_type} with {preprocessing_method}"
            )
            return True
    ...
    def validate_predictions(
        predictions: pd.DataFrame, model_name: str, workflow_component: str
    ) -> bool:
        """
        ...
        try:
            ...
            assert pred_col in predictions.columns, (
            ...
            assert min_val >= VALIDATION_CRITERIA["discharge_min"], (
                f"Predictions too low: {min_val}"
            )
            assert max_val <= VALIDATION_CRITERIA["discharge_max"], (
                f"Predictions too high: {max_val}"
            )

            # Component-specific validation
            if workflow_component in ["calibration", "hindcast"]:
                assert "Q_obs" in predictions.columns, (
                    "Calibration/hindcast should have Q_obs column"
                )
                assert "date" in predictions.columns, (
                    "Calibration/hindcast should have date column"
                )
            elif workflow_component == "operational_prediction":
                assert "forecast_date" in predictions.columns, (
                    "Operational prediction should have forecast_date"
                )
                assert "valid_from" in predictions.columns, (
                    "Operational prediction should have valid_from"
                )
                assert "valid_to" in predictions.columns, (
                    "Operational prediction should have valid_to"
                )

            logger.info(
                f"Prediction validation passed for {model_name} {workflow_component}"
            )
            return True
    ...
    def validate_performance_metrics(
        ...
        try:
            ...
            logger.info(
                f"Performance metrics validation passed for {model_type}: R²={r2:.3f}, RMSE={rmse:.3f}"
            )
            return True
    ...
    def setup(self):
        ...
        try:
            ...
            self.test_dir = tempfile.mkdtemp(
                prefix=f"comprehensive_test_{self.model_type}_{self.preprocessing_method}_"
            )
            ...
            logger.info(
                f"Test environment setup complete for {self.model_type} with {self.preprocessing_method}"
            )
...
def create_mock_patches():
    ...
    patches.append(
        patch(
            "scr.data_utils.glacier_mapper_features", side_effect=lambda df, static: df
        )
    )
...
def calculate_comprehensive_metrics(
    predictions: pd.DataFrame, pred_col: str, obs_col: str = "Q_obs"
) -> Dict[str, float]:
    """
    ...
    try:
        ...
        if len(valid_data) < TEST_CONSTANTS["min_predictions_for_metrics"]:
            logger.warning(
                f"Not enough valid data points for metrics: {len(valid_data)}"
            )
            return {}
