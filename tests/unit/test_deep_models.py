...
class TestDatasetCreation:
    ...
    def test_basic_dataset_creation(self, sample_data, static_data):
        ...
        assert sample["x_future"].shape == (
            37,
            2,
        )  # lookback + future_steps, 2 future features
    ...
    def test_dataset_no_future_features(self, sample_data, static_data):
        ...
        print(
            f"✅ Dataset without future features successful. Dataset size: {len(dataset)}"
        )
    ...
    def test_dataset_no_now_features(self, sample_data, static_data):
        ...
        print(
            f"✅ Dataset without now features successful. Dataset size: {len(dataset)}"
        )
    ...
    def test_nan_handling_strategies(self, sample_data, static_data):
        ...
        assert not torch.isnan(
            sample_mask["x_past"]
        ).any()  # NaN should be filled with 0
        assert sample_mask["x_nan_mask"].sum() >= 0  # Should have some mask values

        print(
            f"✅ NaN handling strategies successful. Mask: {len(dataset_mask)}, Drop: {len(dataset_drop)}"
        )
    ...
    def test_date_range_splitting(self, sample_data, static_data):
        ...
        print(
            f"✅ Date range splitting successful. Train: {len(datamodule.train_dataset)}, "
...
class TestModelArchitectures:
    ...
    def sample_batch(self):
        ...
        batch = {
            ...
            "metadata": [
                {"code": f"basin_{i}", "day": 15, "month": 7, "year": 2023}
    ...
    def test_models_with_missing_inputs(self):
        ...
        model = LSTMForecaster(
            past_dim=2, future_dim=0, static_dim=0, now_dim=0, hidden_dim=16
        )
...
class TestLossFunctions:
    ...
    def test_adaptive_losses(self):
        ...
        adaptive_quantile = AdaptiveQuantileLoss(
            initial_quantiles=[0.1, 0.5, 0.9], learnable=True
        )

        # Adaptive Asymmetric Laplace loss
        adaptive_al = AdaptiveAsymmetricLaplaceLoss(initial_tau=0.5, learnable_tau=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            print(
                "   This is expected in testing - models work with proper data preprocessing"
            )
