...
class TestUtilityFunctions:
    ...
    def test_get_position_name(self):
        ...
        for date, expected in test_cases:
            ...
            assert result == expected, f"Failed for date {date}: expected {expected}, got {result}"
...
class TestTargetCreation:
    ...
    def test_create_target_basic(self):
        ...
        df_with_target = du.create_target(df.copy(), column='discharge', prediction_horizon=30)
    ...
    def test_create_target_different_horizons(self):
        ...
        df = pd.DataFrame({
            'date': dates,
            'code': [1] * 100,
            'discharge': np.arange(100, dtype=float)
        })
        ...
        for horizon in [7, 14, 30]:
            ...
            assert non_nan_count <= (100 - horizon), f"Too many non-NaN values for horizon {horizon}"
    ...
    def test_create_target_multiple_basins(self):
        ...
        df = pd.DataFrame({
            'date': dates,
            'code': [1] * 30 + [2] * 30,
            'discharge': np.concatenate([np.arange(30, dtype=float), np.arange(30, 60, dtype=float)])
        })
...
class TestElevationBandFunctions:
    ...
    def test_get_elevation_bands_per_percentile(self):
        ...
        elevation_data = pd.DataFrame({
            'name': ['elev_1', 'elev_2', 'elev_3', 'elev_4', 'elev_5', 'elev_6', 'elev_7'],
    ...
    def test_calculate_percentile_snow_bands(self):
        ...
        elevation_band_df = pd.DataFrame({
            'code': [1],
            'elevation_bands': [{'Perc_Elev_1': {'bands': [1], 'relative_area': [0.3]}, 
                                'Perc_Elev_2': {'bands': [2], 'relative_area': [0.4]}, 
                                'Perc_Elev_3': {'bands': [3], 'relative_area': [0.3]}}]
        })
        
        try:
            result = du.calculate_percentile_snow_bands(hydro_df, elevation_band_df, num_bands=3)
...
class TestAggregationFunctions:
    ...
    def test_agg_with_min_obs(self):
        ...
        result_insufficient = du.agg_with_min_obs(data_insufficient, func='mean', min_obs=15)
    ...
    def test_create_lag_features(self):
        ...
        assert result['feature1_lag_1'].iloc[1] == 0  # First lag of feature1[1] should be feature1[0]
        assert result['feature1_lag_2'].iloc[2] == 0  # Second lag of feature1[2] should be feature1[0]
...
class TestLongTermMeanFunctions:
    ...
    def test_get_long_term_mean_per_basin(self):
        ...
        for feature in features:
            assert feature in result.columns.get_level_values(0) if hasattr(result.columns, 'get_level_values') else result.columns
    ...
    def test_apply_long_term_mean(self):
        ...
        long_term_mean.columns = pd.MultiIndex.from_tuples([('code', ''), ('month', ''), ('feature1', 'mean')])
