...
class TestGetFeatureImportance:
    ...
    def test_get_feature_importance_xgb(self):
        ...
        assert set(importance_df['feature'].tolist()) == {'feature1', 'feature2', 'feature3'}
...
class TestOptimizeHyperparams:
    ...
    def test_optimize_hyperparams_catboost(self, mock_create_study):
        ...
        result = sci_utils.optimize_hyperparams(
            X_train, y_train, X_val, y_val, 
            model_type='catboost', cat_features=['feature1'], n_trials=5
        )
    ...
    def test_optimize_hyperparams_invalid_model(self):
        ...
        with pytest.raises(ValueError, match="Hyperparameter optimization not supported"):
    ...
    def test_optimize_hyperparams_with_save_path(self, mock_makedirs, mock_create_study):
        ...
        with patch('scr.sci_utils.optuna.visualization') as mock_viz:
            ...
            result = sci_utils.optimize_hyperparams(
                X_train, y_train, X_val, y_val, 
                model_type='xgb', n_trials=5, save_path='/tmp/test'
            )
...
class TestObjectiveFunctions:
    ...
    def test_objective_xgb(self):
        ...
        mock_trial.suggest_float.side_effect = [0.1, 0.8, 0.8, 0.1, 0.1, 0.1]  # learning_rate, subsample, colsample_bytree, gamma, lambda, alpha
    ...
    def test_objective_lgbm(self):
        ...
        # Mock trial
        mock_trial = Mock()
        mock_trial.suggest_int.side_effect = [100, 31, 6, 5]  # n_estimators, num_leaves, max_depth, min_child_samples
        mock_trial.suggest_float.side_effect = [0.1, 0.8, 0.8, 0.1, 0.1]  # learning_rate, subsample, colsample_bytree, lambda_l1, lambda_l2
    ...
    def test_objective_catboost(self):
        ...
        # Mock trial
        mock_trial = Mock()
        mock_trial.suggest_int.side_effect = [100, 6, 128]  # iterations, depth, border_count
        mock_trial.suggest_float.side_effect = [0.1, 0.5, 0.5]  # learning_rate, l2_leaf_reg, bagging_temperature
        
        r2_score = sci_utils._objective_catboost(mock_trial, X_train, y_train, X_val, y_val, [])
    ...
    def test_objective_mlp(self):
        ...
        mock_trial.suggest_categorical.side_effect = ['relu', 'adam', 'constant']  # activation, solver, learning_rate
    ...
    def test_objective_functions_with_normalization(self):
        ...
        with patch('scr.FeatureProcessingArtifacts.post_process_predictions') as mock_post_process:
            # Mock post_process_predictions to return a DataFrame
            mock_post_process.return_value = pd.DataFrame({
                'prediction': np.random.randn(len(y_val)),
                'target': y_val.values
            })
