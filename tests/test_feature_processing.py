...
def create_test_data() -> tuple[pd.DataFrame, pd.DataFrame, List[str], str]:
    ...
    def generate_basin_data(dates: pd.DatetimeIndex, basin_code: str) -> pd.DataFrame:
        ...
        discharge_base = (50 + 30 * seasonal_pattern + np.random.normal(0, 10, n_days)).astype(float)
        discharge_base = np.array(discharge_base, dtype=float)
        discharge_base[missing_indices] = np.nan  # Introduce missing values
        
        precipitation = np.maximum(0, 5 + 3 * seasonal_pattern + np.random.normal(0, 5, n_days))
        ...
        discharge_mean_30 = pd.Series(discharge_base).rolling(30, min_periods=1).mean().values
        ...
        sca_mean_30 = np.maximum(0, np.minimum(100, 80 - 2 * temperature + np.random.normal(0, 10, n_days)))
        
        # Create target (future discharge with some noise) - ensure no NaN values in target
        target_base = np.where(np.isnan(discharge_base), 
                            np.nanmean(discharge_base), 
                            discharge_base)
    ...
    features = [
        'discharge_lag1', 'discharge_lag7', 'discharge_mean_30',
        'P_sum_7', 'P_sum_30', 'T_mean_7', 'T_mean_30',
        'SWE_mean_30', 'SCA_mean_30'
    ]
    target = 'target'
    
    logger.info(f"Created test data: {len(train_df)} training samples, {len(test_df)} test samples")
...
def test_advanced_processing(test_data, advanced_config):
    ...
    assert len(artifacts.final_features) <= advanced_config['number_of_features'] + len(artifacts.cat_features)
...
def test_long_term_mean_handling(test_data, long_term_mean_config):
    ...
    try:
        ...
        logger.info(f"Long-term means created for {len(artifacts.long_term_means)} basins")
...
class TestArtifactPersistence:
    ...
    def test_artifact_saving_loading(self, test_data, basic_config, temp_dir):
        ...
        for format_name in formats:
            ...
            # Load artifacts
            loaded_artifacts = FeatureProcessingArtifacts.load(save_path, format=format_name)
...
def test_configuration_combinations(test_data, missing_strategy, normalize):
    ...
    logger.info(f"=== Testing config: handle_na={missing_strategy}, normalize={normalize} ===")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test FeatureProcessingArtifacts workflow")
    parser.add_argument("--demo", action="store_true", help="Run demo instead of tests")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose logging")
    parser.add_argument("--create-requirements", action="store_true", help="Create requirements-test.txt file")
    ...
    if not PYTEST_AVAILABLE:
        logger.warning("pytest not available. Install with: pip install pytest pytest-cov")
    ...
    if args.demo:
        ...
        if PYTEST_AVAILABLE:
            ...
            logger.info("  pytest test_feature_processing.py --cov=scr.FeatureProcessingArtifacts")
            logger.info("")
