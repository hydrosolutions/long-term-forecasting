...
class TestBaseMetaLearner:
    ...
    def setup_method(self):
        ...
        codes = np.tile(["BASIN01", "BASIN02"], (self.n_samples // 2) + 1)[
            : self.n_samples
        ]
    ...
    def test_calculate_base_model_performance_with_grouping(self):
        ...
        performance = meta_learner.calculate_base_model_performance(
            "XGB", group_by=["code"]
        )
        ...
        assert any(
            "BASIN" in str(key) or isinstance(key, (int, tuple))
    ...
    def test_create_ensemble_predictions_no_weights(self):
        ...
        meta_learner.compute_weights = Mock(
            return_value={"XGB": 1.0, "LGBM": 1.0, "CatBoost": 1.0}
        )
