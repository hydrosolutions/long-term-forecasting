...
def create_model_selector(
    model_options: List[str], default_models: List[str] = None
) -> dcc.Dropdown:
    """
...
def create_metric_selector(
    metric_options: List[str] = None, default_metric: str = "nse"
) -> dcc.Dropdown:
    """
...
def create_basin_selector(
    basin_options: List[str], default_basin: str = None
) -> dcc.Dropdown:
    """
...
def create_month_selector(
    month_options: List[int], default_month: int = None, multi: bool = False
) -> dcc.Dropdown:
    """
...
def create_info_card(title: str, content: str, color: str = "primary") -> dbc.Card:
    ...
    """
    return dbc.Card(
        [dbc.CardHeader(html.H5(title)), dbc.CardBody(html.P(content))],
        color=color,
        outline=True,
        style={"marginBottom": "10px"},
    )
        ...
        columns.append(
            {
                ...
                "type": "numeric",
                "format": {
                    "specifier": METRIC_INFO.get(metric, {}).get("format", ".3f")
                },
            }
        )
...
def create_tab_layout(
    ...
    return html.Div(layout_components, id=f"{tab_id}-layout", style={"padding": "20px"})
