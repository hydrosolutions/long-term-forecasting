...
class MetricsDataHandler:
    ...
    def __init__(
        self,
        metrics_path: str = "../monthly_forecasting_results/evaluation/metrics.csv",
    ):
        """
    ...
    def available_metrics(self) -> List[str]:
        ...
        if self._available_metrics is None:
            ...
            self._available_metrics = [
                col for col in metric_cols if col in self.df.columns
            ]
...
class PredictionDataHandler:
    ...
    def get_observed_vs_predicted(
        ...
        if df.empty:
            logger.warning(
                f"No prediction data available for model {model} and code {code}"
            )
            return df
