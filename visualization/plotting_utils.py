...
def get_color_mapping(df: pd.DataFrame, color_by: str = "model") -> Dict[str, str]:
    ...
    if color_by == "model":
        for model in df['model'].unique():
            family = df[df['model'] == model]['model_family'].iloc[0] if 'model_family' in df.columns else None
...
def highlight_best_performers(df: pd.DataFrame, metric: str, n_best: int = 3) -> List[str]:
    """
