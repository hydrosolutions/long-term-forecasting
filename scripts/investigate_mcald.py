...
def plot_r2_distribution_by_month(r2_df: pd.DataFrame) -> None:
    ...
    plt.title('R² Distribution per Month: Comparison of Prediction Methods\n(Leave-One-Out CV)', fontsize=14)
...
def plot_summary_statistics(r2_df: pd.DataFrame) -> None:
    ...
    summary = r2_df.groupby('month').agg({
    ...
    # Median R² bar chart
    axes[0].bar(x - width, medians_orig, width, label='Q_loc (original)', color='#3498db')
    axes[0].bar(x, medians_mcald, width, label='Q_MC_ALD', color='#e74c3c')
    axes[0].bar(x + width, medians_corr, width, label='Q_loc (corrected)', color='#2ecc71')
    ...
    axes[1].bar(x - width/2, improvement_vs_orig, width, label='vs Q_loc (original)', color='#3498db')
    axes[1].bar(x + width/2, improvement_vs_mcald, width, label='vs Q_MC_ALD', color='#e74c3c')
...
def main() -> None: 
    ...
    logger.info("="*80)
    sample_cols = ['date', 'code', 'month', 'Q_loc', 'Q_MC_ALD', 'Q_loc_corrected', 'Q_obs']
