...
def run_calibration_and_hindcast(model, output_dir: str) -> pd.DataFrame:
    ...
    logger.info(f"Generated {len(hindcast_df)} predictions for {len(hindcast_df['code'].unique())} basins")
...
def calculate_metrics(hindcast_df: pd.DataFrame, output_dir: str, model_name: str) -> pd.DataFrame:
    """
    ...
    for code, group in hindcast_df.groupby(['code']):
        ...
        try:
            metrics = {
                ...
                'nse': metric_functions.nse(obs, pred) if hasattr(metric_functions, 'nse') else np.nan,
            }
            if code == 16936:
...
Examples:
  ...
  python calibrate_hindcast.py --config_dir models/test_model --model_name test_model --skip_metrics
    ...
    parser.add_argument('--model_name', required=True,
                       help='Name of the model to calibrate')
    parser.add_argument('--input_family', required=True,
    ...
    try:
        ...
        if not args.skip_metrics:
            metrics_df = calculate_metrics(hindcast_df, args.output_dir, args.model_name)
