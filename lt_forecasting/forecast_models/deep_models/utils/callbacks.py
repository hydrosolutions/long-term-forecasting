...
class DeepForecastingCallbacks:
        ...
        **kwargs,
        ...
        if log_lr:
            lr_monitor = LearningRateMonitor(
                logging_interval="epoch", **kwargs.get("lr_monitor_params", {})
            )
    ...
    def get_uncertainty_callbacks(
        save_dir: str, model_name: str, monitor: str = "val_loss", **kwargs
    ) -> List[pl.Callback]:
        """
...
class ForecastingMetricsCallback(pl.Callback):
    ...
    def _compute_forecasting_metrics(
        self, trainer: pl.Trainer, pl_module: pl.LightningModule
    ):
        ...
        all_predictions = torch.cat(
            [x["predictions"] for x in pl_module.validation_outputs]
        )
...
def get_default_callbacks(
    save_dir: str, model_name: str, model_type: str = "standard", **kwargs
) -> List[pl.Callback]:
    """
    ...
    if model_type == "meta_learning":
        callbacks = DeepForecastingCallbacks.get_meta_learning_callbacks(
            save_dir=save_dir, model_name=model_name, **kwargs
        )
