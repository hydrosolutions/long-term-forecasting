...
class PerformanceTracker:
    ...
    def __init__(self, track_basin_specific: bool = True, track_temporal: bool = True):
        ...
        logger.info(f"Initialized PerformanceTracker with basin_specific={track_basin_specific}, temporal={track_temporal}")
    ...
    def get_basin_performance(
        ...
        """
        if (model_id not in self.basin_performance or 
            basin_code not in self.basin_performance[model_id]):
            return []
    ...
    def get_temporal_performance(
        ...
        """
        if (model_id not in self.temporal_performance or 
            month not in self.temporal_performance[model_id]):
            return []
    ...
    def compare_models(
        ...
        for model_id in model_ids:
            ...
            if comparison_type == 'latest':
                comparison_results[model_id] = records[-1]['overall_metrics'].get(metric, np.nan)
    ...
    def get_performance_summary(
        ...
        if include_basin and self.track_basin_specific and model_id in self.basin_performance:
            basin_summary = {}
        ...
        # Temporal summary
        if include_temporal and self.track_temporal and model_id in self.temporal_performance:
            temporal_summary = {}
...
def calculate_performance_weights(
    ...
    valid_performance = {
        model_id: perf for model_id, perf in performance_data.items()
...
def analyze_performance_stability(
    ...
    values = [record['overall_metrics'].get(metric, np.nan) for record in recent_records]
    values = [v for v in values if not np.isnan(v)]
