...
def r2_score(
    observed: Union[np.ndarray, pd.Series], predicted: Union[np.ndarray, pd.Series]
) -> float:
    """
...
def kge(
    ...
    try:
        ...
        # Calculate KGE
        kge_value = 1 - np.sqrt(
            (correlation - 1) ** 2 + (bias_ratio - 1) ** 2 + (var_ratio - 1) ** 2
        )
...
def evaluate_predictions(
    ...
    if group_cols is None:
        ...
        metrics = calculate_all_metrics(
            predictions_df[observed_col], predictions_df[predicted_col]
        )
        ...
        def calculate_group_metrics(group_df):
            return pd.Series(
                calculate_all_metrics(group_df[observed_col], group_df[predicted_col])
            )

        return (
            predictions_df.groupby(group_cols)
