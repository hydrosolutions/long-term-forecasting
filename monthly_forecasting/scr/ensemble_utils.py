...
class EnsembleBuilder:
    ...
    def __init__(self, ensemble_method: str = "mean", handle_missing: str = "skip"):
        ...
        if ensemble_method not in valid_methods:
            raise ValueError(
                f"Invalid ensemble method: {ensemble_method}. Must be one of {valid_methods}"
            )
        ...
        if handle_missing not in valid_missing:
            raise ValueError(
                f"Invalid missing handling: {handle_missing}. Must be one of {valid_missing}"
            )

        logger.info(
            f"Initialized EnsembleBuilder with method={ensemble_method}, missing={handle_missing}"
        )
    ...
    def create_simple_ensemble(
        ...
        logger.info(
            f"Created simple ensemble with {len(ensemble_df)} predictions using {self.ensemble_method}"
        )
        ...
        if metric_type == "error":
            ...
            for model_id in predictions_dict.keys():
                if model_id in performance_metrics:
                    perf_value = performance_metrics[model_id]
                    if (
                        not np.isnan(perf_value)
                        and np.isfinite(perf_value)
                        and perf_value > 0
                    ):
    ...
    def create_adaptive_ensemble(
        ...
        # Align predictions
        aligned_predictions = align_predictions(
            predictions_dict, alignment_method="inner"
        )
        ...
        for idx, row in ensemble_df.iterrows():
            ...
            if total_weight > 0:
                context_weights = {
                    k: v / total_weight for k, v in context_weights.items()
                }
            elif fallback_uniform:
                context_weights = {
                    k: 1.0 / len(context_weights) for k in context_weights.keys()
                }
...
def create_family_ensemble(
    ...
    if not ensemble_df.empty:
        ...
        logger.info(
            f"Created {family_name} family ensemble with {len(ensemble_df)} predictions"
        )
...
def create_global_ensemble(
    ...
    if not ensemble_df.empty:
        ...
        logger.info(
            f"Created global ensemble with {len(ensemble_df)} predictions from {len(predictions_dict)} models"
        )
...
def evaluate_ensemble_performance(
    ...
    for metric in metrics:
        if metric in ensemble_metrics:
            ...
            base_values = [
                base_metrics[model_id].get(metric, np.nan)
                for model_id in base_metrics.keys()
            ]
            ...
            if base_values:
                best_base = (
                    ...
                    else min(base_values)
                )
                avg_base = np.mean(base_values)

                if metric in ["r2", "nse", "kge"]:
                    # Higher is better
                    improvements[f"{metric}_vs_best"] = (
                        (ensemble_value - best_base) / best_base * 100
                        if best_base != 0
                        else 0
                    )
                    improvements[f"{metric}_vs_avg"] = (
                        (ensemble_value - avg_base) / avg_base * 100
                        if avg_base != 0
                        else 0
                    )
                else:
                    # Lower is better
                    improvements[f"{metric}_vs_best"] = (
                        (best_base - ensemble_value) / best_base * 100
                        if best_base != 0
                        else 0
                    )
                    improvements[f"{metric}_vs_avg"] = (
                        (avg_base - ensemble_value) / avg_base * 100
                        if avg_base != 0
                        else 0
                    )
...
def optimize_ensemble_weights(
    ...
    for weights_tuple in product(weight_options, repeat=len(model_ids)):
        ...
        normalized_weights = {
            model_ids[i]: weights_tuple[i] / total_weight for i in range(len(model_ids))
        }

        # Create ensemble with these weights
        ensemble_builder = EnsembleBuilder(ensemble_method="weighted_mean")
        ensemble_df = ensemble_builder.create_weighted_ensemble(
            predictions_dict, normalized_weights
        )
    ...
    logger.info(
        f"Optimized ensemble weights: {best_weights}, best {optimization_metric}: {best_score:.4f}"
    )
...
def create_ensemble_summary(
    ensemble_predictions: pd.DataFrame, base_predictions: Dict[str, pd.DataFrame]
) -> pd.DataFrame:
    """
