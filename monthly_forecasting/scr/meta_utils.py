...
def validate_prediction_format(predictions: pd.DataFrame, model_id: str = None) -> bool:
    ...
    if missing_columns:
        raise ValueError(
            f"Missing required columns in predictions{' for ' + model_id if model_id else ''}: {missing_columns}"
        )

    # Check for empty DataFrame
    if predictions.empty:
        raise ValueError(
            f"Empty predictions DataFrame{' for ' + model_id if model_id else ''}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid date format in predictions{' for ' + model_id if model_id else ''}: {e}"
        )
    ...
    for col in numeric_columns:
        if not pd.api.types.is_numeric_dtype(predictions[col]):
            raise ValueError(
                f"Column {col} must be numeric in predictions{' for ' + model_id if model_id else ''}"
            )
...
def align_predictions(
    ...
    logger.info(
        f"Common index has {len(common_index)} entries after {alignment_method} alignment"
    )
    ...
    for model_id, predictions in predictions_dict.items():
        ...
        aligned_df = pd.DataFrame(index=common_index, columns=["Q_obs", "Q_pred"])
...
def create_meta_features(
    ...
    for idx, row in common_df.iterrows():
        ...
        feature_dict = {"date": date, "code": code, "Q_obs": obs}
        ...
        if len(model_predictions) > 1:
            ...
            if len(pred_array) > 0:
                ...
                if len(pred_array) > 1:
                    feature_dict["pred_cv"] = (
                        np.std(pred_array) / np.mean(pred_array)
                        if np.mean(pred_array) != 0
                        else 0
                    )
                ...
                if isinstance(perf_dict, dict):
                    for metric_name, metric_value in perf_dict.items():
                        if not np.isnan(metric_value):
                            feature_dict[f"perf_{model_id}_{metric_name}"] = (
                                metric_value
                            )
    ...
    logger.info(
        f"Created meta-features with {len(meta_features_df)} rows and {len(meta_features_df.columns)} columns"
    )
...
def ensemble_predictions(
    ...
    logger.info(
        f"Created ensemble with {len(ensemble_df)} predictions using {method} method"
    )
...
def calculate_model_agreement(
    predictions_dict: Dict[str, pd.DataFrame], metric: str = "correlation"
) -> pd.DataFrame:
    """
    ...
    for i, model_i in enumerate(model_ids):
        for j, model_j in enumerate(model_ids):
            if i == j:
                agreement_matrix.loc[model_i, model_j] = (
                    1.0 if metric == "correlation" else 0.0
                )
            else:
                ...
                if len(pred_i_clean) > 1:
                    if metric == "correlation":
                        agreement_matrix.loc[model_i, model_j] = np.corrcoef(
                            pred_i_clean, pred_j_clean
                        )[0, 1]
                    elif metric == "rmse":
                        agreement_matrix.loc[model_i, model_j] = np.sqrt(
                            np.mean((pred_i_clean - pred_j_clean) ** 2)
                        )
                    elif metric == "mae":
                        agreement_matrix.loc[model_i, model_j] = np.mean(
                            np.abs(pred_i_clean - pred_j_clean)
                        )
                else:
...
def analyze_prediction_diversity(
    predictions_dict: Dict[str, pd.DataFrame], diversity_metrics: List[str] = None
) -> Dict[str, float]:
    """
    ...
    for metric in diversity_metrics:
        if metric == "std":
            ...
            diversity_results[metric] = np.nanmean(
                np.nanmax(pred_matrix, axis=1) - np.nanmin(pred_matrix, axis=1)
            )
        elif metric == "iqr":
            ...
            diversity_results[metric] = np.nanmean(
                np.nanstd(pred_matrix, axis=1) / np.nanmean(pred_matrix, axis=1)
            )
...
def evaluate_meta_learning_performance(
    ...
    baseline_ensemble = ensemble_predictions(predictions_dict, method=baseline_method)
    ...
    baseline_metrics = calculate_all_metrics(
        baseline_ensemble["Q_obs"], baseline_ensemble["Q_pred"]
    )
