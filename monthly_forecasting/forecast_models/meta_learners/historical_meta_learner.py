...
class HistoricalMetaLearner(BaseMetaLearner):
    ...
    def __get_weights__(self, performance: pd.DataFrame) -> pd.DataFrame:
        ...
        model_columns = [col for col in performance.columns if col not in ['code', 'period']]
        ...
        logger.info(f"Weights calculated for {len(weights_df)} code-period combinations")
    ...
    def __loocv__(self, loocv_years: List[int]) -> pd.DataFrame:
        ...
        for year in loocv_years:
            ...
            historical_performance = self.__calculate_historical_performance__(train_data, model_names)
            ...
            ensemble_predictions = self.__create_ensemble__(val_data, model_names, weights)
            ...
            logger.info(f"Completed LOOCV for year {year}: {len(ensemble_predictions)} predictions")
    ...
    def predict_operational(self, today: datetime.datetime = None) -> pd.DataFrame:
        ...
        weights_path = os.path.join(self.path_config.get("model_home_path", ""), f"{self.name}_weights.parquet")
        ...
        if os.path.exists(weights_path):
            ...
            if len(historical_performance) == 0:
                logger.error("No historical performance data available for operational prediction")
                return pd.DataFrame()
        ...
        if len(today_data) == 0:
            logger.warning(f"No data available for {today.date()}, using most recent data")
        ...
        logger.info(f"Operational prediction completed: {len(operational_predictions)} predictions")
    ...
    def calibrate_model_and_hindcast(self) -> pd.DataFrame:
        ...
        performance_path = os.path.join(self.path_config.get("model_home_path", ""), f"{self.name}_performance.parquet")
        ...
        logger.info(f"Calibration completed: {len(hindcast_predictions)} hindcast predictions")
    ...
    def tune_hyperparameters(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(available_years) < 3:
                return False, "Insufficient data for hyperparameter tuning (need at least 3 years)"
            ...
            for num_samples in num_samples_vals:
                for metric in metrics_to_test:
                    logger.info(f"Testing num_samples_val={num_samples}, metric={metric}")
                    ...
                    try:
                        ...
                        if len(historical_performance) == 0:
                            logger.warning(f"No performance data for params: num_samples={num_samples}, metric={metric}")
                            continue
                        ...
                        val_predictions = self.__create_ensemble__(val_data, model_names, weights)
                        
                        # Calculate validation score (using NSE as evaluation metric)
                        from monthly_forecasting.scr.metrics import calculate_NSE
                        
                        if 'Q_obs' in val_predictions.columns and 'ensemble' in val_predictions.columns:
                            ...
                            if np.sum(valid_mask) >= 10:
                                score = calculate_NSE(observed[valid_mask], predicted[valid_mask])
                                ...
                                if not np.isnan(score) and score > best_score:
                                    ...
                                    logger.info(f"New best score: {score:.4f} with params: {best_params}")
                    
                    except Exception as e:
                        logger.warning(f"Error testing params num_samples={num_samples}, metric={metric}: {str(e)}")
            ...
            if best_params:
                ...
                logger.info(f"Hyperparameter tuning completed. Best params: {best_params}, Score: {best_score:.4f}")
