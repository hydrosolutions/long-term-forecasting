...
class DeepLearningDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(
            f"Past features: {len(self.past_features)}, Future features: {len(self.future_features)}"
        )
        logger.info(
            f"Static features: {len(self.static_features)}, Now features: {len(self.now_features)}"
        )
    ...
    def _is_valid_sample(self, start_i: int, today_i: int, end_i: int) -> bool:
        ...
        if self.drop_nan_samples:
            ...
            if (
                ...
                and np.isnan(self.future_array[start_i:end_i]).any()
            ):
                return False
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.past_features:
            x_past = self.past_array[
                start_i:today_i
            ].copy()  # (lookback, n_past_features)
        ...
        if self.future_features:
            x_future = self.future_array[
                start_i:end_i
            ].copy()  # (lookback + future_steps, n_future_features)

            if self.use_nan_mask:
                x_future = np.nan_to_num(x_future, nan=0.0)
        else:
            x_future = np.empty(
                (self.lookback + self.future_known_steps, 0), dtype=np.float32
            )
