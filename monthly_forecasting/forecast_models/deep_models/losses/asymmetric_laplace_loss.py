...
class AsymmetricLaplaceLoss(nn.Module):
    ...
    def __init__(self, tau: float = 0.5, reduction: str = 'mean'):
        ...
        logger.info(f"AsymmetricLaplaceLoss initialized with tau={tau}, reduction={reduction}")
    ...
    def forward(
        ...
        if mu.shape != targets.shape:
            raise ValueError(f"mu and targets must have same shape, got {mu.shape} and {targets.shape}")
        if b.shape != targets.shape:
            raise ValueError(f"b and targets must have same shape, got {b.shape} and {targets.shape}")
...
class RobustAsymmetricLaplaceLoss(AsymmetricLaplaceLoss):
    ...
    def __init__(
        ...
        logger.info(f"RobustAsymmetricLaplaceLoss initialized with "
    ...
    def forward(
        ...
        if self.huber_delta is not None:
            ...
            smooth_residual = torch.where(
                huber_mask,
                residual,
                torch.sign(residual) * (self.huber_delta + 
                                      torch.log(1 + abs_residual - self.huber_delta))
            )
