...
class QuantileLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if targets.dim() == 1:
            ...
            raise ValueError(f"Targets must have shape (batch_size, 1), got {targets.shape}")
        ...
        quantile_levels = self.quantile_levels.expand(batch_size, -1)  # (batch_size, num_quantiles)
    ...
    def compute_interval_score(
        ...
        lower_idx = min(range(self.num_quantiles), 
                       key=lambda i: abs(self.quantiles[i] - lower_quantile))
        upper_idx = min(range(self.num_quantiles), 
