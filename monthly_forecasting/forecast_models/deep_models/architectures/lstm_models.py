...
class LSTMForecaster(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        final_input_dim = lstm_output_dim + future_output_dim + now_output_dim + static_output_dim
        ...
        logger.info(f"LSTMForecaster initialized with final_input_dim={final_input_dim}")
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, lstm_outputs: torch.Tensor) -> torch.Tensor:
        ...
        attention_weights = torch.tanh(self.attention_fc(lstm_outputs))  # (batch, seq_len, attention_dim)
        attention_scores = torch.matmul(attention_weights, self.context_vector)  # (batch, seq_len)
        attention_scores = F.softmax(attention_scores, dim=1)  # (batch, seq_len)
        
        # Apply attention weights
        attended_features = torch.sum(
            lstm_outputs * attention_scores.unsqueeze(-1), 
            dim=1
        )  # (batch, hidden_dim)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if 'x_past' in batch:
            ...
            for i, lstm_layer in enumerate(self.lstm_layers):
                ...
                if self.use_skip_connections and i > 0 and layer_input.size(-1) == layer_output.size(-1):
