...
class CNNLSTMForecaster(nn.Module):
        ...
        **kwargs,
        ...
        if now_dim > 0:
            self.now_fc = nn.Sequential(
                nn.Linear(now_dim, lstm_hidden_dim), nn.ReLU(), nn.Dropout(dropout)
            )
        ...
        if static_dim > 0:
            self.static_fc = nn.Sequential(
                nn.Linear(static_dim, lstm_hidden_dim), nn.ReLU(), nn.Dropout(dropout)
            )
        ...
        final_input_dim = (
            past_output_dim + future_output_dim + now_output_dim + static_output_dim
        )
        ...
        logger.info(
            f"CNNLSTMForecaster initialized with final_input_dim={final_input_dim}"
        )
    ...
    def _calculate_cnn_output_length(
        self, input_length: int, kernel_sizes: List[int], pool_sizes: List[int]
    ) -> int:
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if (
            self.cnn_layers is not None
            and self.past_lstm is not None
            and "x_past" in batch
        ):
            ...
            cnn_output = self.cnn_layers(
                cnn_input
            )  # (batch, cnn_filters[-1], reduced_length)

            # Reshape for LSTM: (batch, reduced_length, cnn_filters[-1])
            cnn_output = cnn_output.transpose(
                1, 2
            )  # (batch, reduced_length, cnn_filters[-1])
        ...
        if (
            self.future_cnn is not None
            and self.future_lstm is not None
            and "x_future" in batch
        ):
            ...
            # Reshape for LSTM
            future_cnn_output = future_cnn_output.transpose(
                1, 2
            )  # (batch, reduced_length, filters)
...
class CNNLSTMLightningModel(LightningForecastBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"CNNLSTMLightningModel initialized with architecture: {architecture}"
        )
