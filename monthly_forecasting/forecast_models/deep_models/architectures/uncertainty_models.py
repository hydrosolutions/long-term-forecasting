...
class UncertaintyNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"UncertaintyNet initialized with final_input_dim={final_input_dim}")
...
class UncertaintyLightningModel(LightningMetaForecastBase):
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        loss = self._asymmetric_laplace_loss(mu.squeeze(-1), b.squeeze(-1), targets, self.tau)
    ...
    def _asymmetric_laplace_loss(self, mu: torch.Tensor, b: torch.Tensor, y: torch.Tensor, tau: float) -> torch.Tensor:
        ...
        loss = torch.where(
            ...
            tau * diff / b,
            (tau - 1) * diff / b
        ) + torch.log(b / (tau * (1 - tau)))
        
        return loss.mean()

    def _predict_quantile_torch(self, mu: torch.Tensor, b: torch.Tensor, tau: float, q: float) -> torch.Tensor:
