...
class DeepRegressor(BaseForecastModel):
    ...
    def _train_model(
        self, train_dataset: DeepLearningDataset, test_year: int
    ) -> pl.LightningModule:
    ...
    def _predict_with_model(
        self, model: pl.LightningModule, dataset: DeepLearningDataset, year: int
    ) -> pd.DataFrame:
    ...
    def load_model(self) -> None:
        ...
        if os.path.exists(artifacts_path):
            self.feature_processing_artifacts = (
                FeatureProcessingArtifacts.load_from_disk(artifacts_path)
            )
