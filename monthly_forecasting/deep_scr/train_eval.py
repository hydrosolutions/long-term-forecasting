...
def normalize_train_data(
    ...
    df_train[discharge_col] = grp_disc.transform(
    ...
    for id in df_train[id_col].unique():
        ...
        for col in base_learner_cols:
            if col in df_train.columns:
                df_train.loc[df_train[id_col] == id, col] = (
                    df_train.loc[df_train[id_col] == id, col] - mean
                ) / std
            else:
                logger.warning(
                    f"Column {col} not found in DataFrame. Skipping scaling for this column."
                )
...
def scale_data(
    ...
    df_scaled[target_col] = df_scaled.apply(
        lambda row: _scale_val(row[target_col], row[id_col], scaler_target), axis=1
    )
    ...
    for id in df_scaled[id_col].unique():
        ...
        for col in base_learner_cols:
            if col in df_scaled.columns:
                df_scaled.loc[df_scaled[id_col] == id, col] = (
                    df_scaled.loc[df_scaled[id_col] == id, col] - mean
                ) / std
            else:
                logger.warning(
                    f"Column {col} not found in DataFrame. Skipping scaling for this column."
...
def scale_static_features(static_df: pd.DataFrame, id_col: str = None):
    """
...
def get_error_distribution_of_base_learner(
    ...
    dist = (
        dfc.groupby([id_col, "month"])[f"error"]
        .agg(
            mean_error="mean",
...
def apply_error_distribution(
    ...
    # merge stats
    merged = pd.merge(dfc, error_distribution, on=[id_col, "month"], how="left")
...
def get_nse_distribution_of_base_learner(
    ...
    nse_dist = (
        dfc.groupby([id_col, "unique_time"])
        .apply(safe_nse)
...
def apply_nse_distribution(
    ...
    merged = pd.merge(dfc, nse_distribution, on=[id_col, "unique_time"], how="left")
...
def prepare_data(df_train, df_val, df_test, static_df, save=False):
    ...
    # Scale the data
    df_train, scaler_discharge, scaler_target, scaler_features = normalize_train_data(
        df_train
    )
    df_val = scale_data(
        df_val, scaler_discharge, scaler_target, scaler_features, id_col="code"
    )
    df_test = scale_data(
        df_test, scaler_discharge, scaler_target, scaler_features, id_col="code"
    )
...
def calibrate(model, dm, load_ckpt=True):
    ...
    early_stop_cb = EarlyStopping(
        monitor="val_loss", patience=META_CONFIG["early_stop_patience"], mode="min"
    )
...
def get_predictions(model, dm, trainer, scaler_target):
    ...
    for code in predictions["code"].unique():
        ...
        predictions.loc[predictions["code"] == code, pred_cols] = (
            predictions.loc[predictions["code"] == code, pred_cols] * std_ + mean_
        )
