...
class MetaMonthDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_ds = META_MONTH_DATA(self.train_df, self.static_df, **self.dataset_kwargs)
        self.val_ds = META_MONTH_DATA(self.val_df, self.static_df, **self.dataset_kwargs)
        self.test_ds = META_MONTH_DATA(self.test_df, self.static_df, **self.dataset_kwargs)
...
class META_MONTH_DATA(Dataset):
    def __init__(self,
        ...
        for _, row in self.static_df.iterrows():
            ...
            self.static_features_dict[code] = row[self.static_features].values.astype(np.float32)
